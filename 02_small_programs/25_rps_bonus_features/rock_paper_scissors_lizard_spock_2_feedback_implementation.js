...
function displayUserWins(userMove, computerMove) {
  console.log(`\n`);
  prompt(`User Wins!`);
...
function displayComputerWins(computerMove, userMove) {
  console.log(`\n`);
...
function getGrandWinner(userPlayer, computerPlayer) {
  let grandWinner;
  if (userPlayer.getScore() === GRAND_WINNING_SCORE) {
    grandWinner = `You Won Best Of 5 Games! You are the Grand Winner!`;
  } else if (computerPlayer.getScore() === GRAND_WINNING_SCORE) {
    grandWinner = `Computer Won Best Of 5 Games! Computer is the Grand Winner!`;
  }
...
(function playGame() {
  const userPlayer = createScoreInterface();
  const computerPlayer = createScoreInterface();
  ...
  while (true) {
    ...
    if (playerWins(userMove, computerMove)) {
      displayUserWins(userMove, computerMove);
      userPlayer.increaseScore();
    } else if (playerWins(computerMove, userMove)) {
      displayComputerWins(computerMove, userMove);
    ...
    if (getGrandWinner(userPlayer, computerPlayer)) {
      prompt(getGrandWinner(userPlayer, computerPlayer));
